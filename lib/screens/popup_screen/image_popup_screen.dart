...
class _LiquidImagePopupState extends State<LiquidImagePopup>
  ...
  void initState() {
    ...
    _slidePopup = Tween<Offset>(begin: const Offset(0, -0.3), end: Offset.zero)
        .animate(
    ...
    _tapSlide =
        Tween<Offset>(begin: const Offset(0, 0), end: const Offset(0, 0.05))
            .animate(
    ...
    _tapShrinkController =
        AnimationController(vsync: this, duration: const Duration(milliseconds: 200));
    _imageScale = Tween<double>(begin: 1.0, end: 0.95).animate(
        CurvedAnimation(parent: _tapShrinkController, curve: Curves.easeOut));
  }
  ...
  Future<void> _launchWebsite() async {
    ...
    if (await canLaunchUrl(Uri.parse(widget.websiteUrl))) {
      await launchUrl(Uri.parse(widget.websiteUrl),
            ...
            return Container(
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: [
                    isDark
                        ? Colors.black.withOpacity(0.6 + 0.1 * (_backgroundController.value))
                        : Colors.white.withOpacity(0.2 + 0.05 * (_backgroundController.value)),
                    isDark
                        ? Colors.deepPurple.withOpacity(0.4 + 0.1 * (_backgroundController.value))
                        : Colors.blue.withOpacity(0.1 + 0.05 * (_backgroundController.value)),
                  ],
        ...
        Positioned(
          ...
          child: IconButton(
            icon: Icon(Icons.close_rounded,
                color: isDark ? Colors.white70 : Colors.black87, size: 30),
