...
-- We used the GUI to create the tables, set constraints and define RLS (Row-level security) policies for RBAC (Role-based Access Control)
...
CREATE TABLE public.Popups (
  ...
  start_timestamp timestamp without time zone NOT NULL DEFAULT (now() AT TIME ZONE 'utc-5:30'::text),
  end_timestamp timestamp without time zone NOT NULL,
  link character varying,
  target_insti uuid NOT NULL DEFAULT auth.uid(),
